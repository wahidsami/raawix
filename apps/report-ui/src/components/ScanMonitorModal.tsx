...
interface ScanEvent {
  ...
  meta?: { title?: string; findingsCount?: number; visionCount?: number };
  timestamp: string;
  [key: string]: any;
}
...
export default function ScanMonitorModal({ scanId, seedUrl, scanMode = 'domain', maxPages, maxDepth, entityId, propertyId, onClose, onComplete }: ScanMonitorModalProps) {
  ...
  const handleLayerStatus = (event: ScanEvent) => {
    ...
    if (status === 'running') {
      ...
      if (docTitle) {
        setActiveScanPageTitle(docTitle);
      }
    }
    ...
    setTree((prev) => {
      ...
      if (!newTree.has(url)) {
        ...
        const metaTitle = typeof event.meta?.title === 'string' ? event.meta.title.trim() : '';
